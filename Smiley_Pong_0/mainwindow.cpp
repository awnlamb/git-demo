    ...
    ui(new Ui::MainWindow)
{
    ...
    //player->setPos((width()/2.0)+(player->width())/2.0, 500);
    player->setPos((width()/2.0)-(player->width())/2.0,500);
    ...
    //ball->setPos(player->x() + player->height() / 2, player->y() + ball->width() + 12);
    ball->setPos(player->x()+player->width()/2, player->y()-ball->height()-12);
    //ball->setPos(800,300);
    ...
    //ui->label_2->setText(QString::number(ball->point()));
    ui->label_2->setText(QString("%1").arg(ball->point()));
    if(ball->y()>600) ui->label_3->setVisible(1);
}
...
void MainWindow::on_actionHow_to_play_triggered()
{
    ...
    msgBox->setText("Первый игрок - управление клавишами W и S, второй игрок - управление стрелками Вверх и Вниз");
